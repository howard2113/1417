...
'''
SectionID	RoadName	RoadDirection	SectionStart.PositionLat	SectionStart.PositionLon	SectionEnd.PositionLat	SectionEnd.PositionLon		路段起點	路段終點
L_1008801102000E	台88線	W						台88線匝道出口	鳳頂路

'''


class SectionVD(db.Model):
    __tablename__ = 'road_section_vd'
...
class RoadDynamicFlow(db.Model):
    ...
    def __init__(self, id, l_id, area_name, name, dev, pcu, h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, h15, h16, h17, h18, h19, h20, h21, h22, h23, src_time):
        self.id = id
        self.l_id = l_id
        self.area_name = area_name
        self.name = name
...
class RoadDynamicFlowSchema(ModelSchema):
    class Meta(ModelSchema.Meta):
        model = RoadDynamicFlow
        sqla_session = db.session
    ...
    area_name = fields.String(required=True)
    name = fields.String(required=True)
    dev = fields.Integer(required=True)
    pcu = fields.Float(required=True)
