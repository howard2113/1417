...
r_dynamic_flow_df = pd.read_sql(f"SELECT * FROM road_dynamic_flow", con=engine)
last_time_minute = -1
while True:
    now = datetime.now()
    ...
    # if now.minute in update_minute_list and now.minute != last_time_minute or True: # for dev
        ...
        for index, vd_row in vd_road_static_df.iterrows():
            if len(r_dynamic_flow_df[r_dynamic_flow_df['id'] == vd_row['id']].index) <= 0:
                r_dynamic_flow_df = r_dynamic_flow_df.append({'id': vd_row['id'], 'area_name': vd_row['area_name'], 'name': vd_row['name'], 'pcu': -1, 'h0': -1, 'h1': -1, 'h2': -1,
            ...
            if now.hour == 0 and now.minute == 0:
                r_dynamic_flow_df.loc[(r_dynamic_flow_df['id'] == vd_row['id'], hour_map)] = -1

            for hour in range(24):
                if now.hour >= hour:
                    flow = random.randrange(10, 5000)
                    r_dynamic_flow_df.loc[(r_dynamic_flow_df['id'] == vd_row['id']), hour_map[hour]] = flow
                    r_dynamic_flow_df.loc[(r_dynamic_flow_df['id'] == vd_row['id']), 'pcu'] = flow * 2.2

        engine.execute("DELETE FROM road_dynamic_flow")
        r_dynamic_flow_df.to_sql('road_dynamic_flow', engine, if_exists='append', index=False, chunksize=500, )
